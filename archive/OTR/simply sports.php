...
<body>
    ...
    <div id="toc">
    <div style="height: 57px; opacity:0;">spacer</div>
    <h2>Jump To...</h2>
    </div>
    <div id="maincontent">
        <div id="title">
            ...
            foreach ($files as $file) {
                if ($file != '.' && $file != '..') {
                        ...
                        if (!file_exists($filepath)){
                            ...
                            $now = new DateTime();
                            $uploadDate = new DateTime(date($date." ".$time));
                            if ($now > $uploadDate) {
    ...
    for(i = 0; i < posts.length; i++){
        document.getElementById("toc").innerHTML += "<a href='#" + posts[i].id + "'>Simply Sports " + posts[i].id + "</a><br>";
    }
    
    test = function () {
        for(i = 0; i < posts.length; i++){
            document.getElementById("toc").getElementsByTagName("a")[i].style.color = "#0047ab";
        }
        for(i = posts.length - 1; i >= 0; i--){
            if(posts[i].offsetTop <= $(window).scrollTop() + posts[0].offsetTop){
                document.getElementById("toc").getElementsByTagName("a")[i].style.color = "#003279";
                break;
            }
        }
    }
